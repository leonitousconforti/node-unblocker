...
function urlPrefixer(config) {
    var re_abs_url = /("|'|=|url\(\s*)(https?:)/ig, // "http:, 'http:, =http:, or url( http:, also matches https versions
        ...
        re_js_websocket = new RegExp('(var|let)?\\\s*([A-Za-z_\\\-{0-9}]+)\\\s*=\\\s*(new websocket\\\s*\\\(([A-Za-z_()\\\"\\\'\\\-\\\s\\\+{0-9}/:.]*)\\\s*\\\);)', 'i', 'm', 'g'),
    ...
    function rewriteUrls(chunk, uri, prefix) {
        ...
        // finaly update websocket urls
        var test = chunk.match(re_js_websocket);
        if (test != null) {
          chunk = chunk.replace(test[4], "\"" + uri.protocol + "//" + uri.host + prefix + "wss://\" \+ " + test[4]);
        }
