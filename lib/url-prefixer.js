...
function urlPrefixer(config) {
    var re_abs_url = /("|'|=|url\(\s*)(https?:)/ig, // "http:, 'http:, =http:, or url( http:, also matches https versions
        ...
        // in order to detect all types of websockets, we search for the 'new websocket()' constructor instead of 'wss://' or 'ws://' because those are not used in all websites
        // for example, agar.io does 'new websocket(a(b))' and slither.io does similar so it wouldn't find 'wss://'
        re_js_websocket = new RegExp('(var |let |this.)?\\\s*([A-Za-z_\\\-{0-9}]+)\\\s*=\\\s*(new websocket\\\s*\\\(([A-Za-z_()\\\"\\\$\\\'\\\-\\\s\\\+{0-9}/:.]*)\\\s*\\\)(;?,?))', 'i', 'm', 'g'),
    ...
    function rewriteWebsockets(chunk, prefix, websocketServer) {
      chunk = chunk.replace(re_js_websocket, "$1$2 = new WebSocket(\"" + websocketServer + prefix + "\" \+ $4)$5");
      return chunk;
    }

    function handleWebsocketsStream() {
        return new Transform({
            decodeStrings: false,

            transform: function(chunk, encoding, next) {
                chunk = chunk.toString();
                chunk = rewriteWebsockets(chunk, config.prefix, config.websocketServer);
    ...
    function prefixUrls(data) {
        if ((contentTypes.isJs(config, data)) && (config.proxyWebSockets !== false)) {
            debug('handling websockets in JS files only');
            data.stream = data.stream.pipe(handleWebsocketsStream());
        } else if (contentTypes.shouldProcess(config, data)) {
    ...
    prefixUrls.rewriteWebsockets = rewriteWebsockets; // for testing
